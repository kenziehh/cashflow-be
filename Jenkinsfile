pipeline {
    ...
    environment {
        IMAGE_NAME = 'secure-cashflow-be-app'

        // Staging Ports
        STAGING_APP_PORT = '8081'
    ...
    stages {
        stage('Checkout') {
            steps {
                git url: 'https://github.com/kenziehh/cashflow-be.git', branch: 'main'
            }
        }
...
CORS_ALLOWED_ORIGINS=http://localhost:3000,https://cashflow-fe.nflrmvs.cloud,https://cashflow.nflrmvs.cloud
ENVIRONMENT=staging
EOF
...
services:
  app:
    ...
    container_name: cashflow-be-staging
  postgres:
    container_name: cashflow-db-staging
  redis:
    container_name: redis-cashflow-staging
EOF
